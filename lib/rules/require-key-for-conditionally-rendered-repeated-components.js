...
const reservedNamesInVue3 = require('../utils/vue3-builtin-components')
...
const allContainerElements = [...reservedNamesInVue3, ...htmlContainerElements]
...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      categories: null,
      recommended: false,
      url: 'https://eslint.vuejs.org/rules/require-key-for-conditionally-rendered-repeated-components.html'
    },
    fixable: 'code',
    schema: [],
    messages: {
  ...
  create(context) {
    ...
    /** Array of built-in Vue components that are exempt from the rule. */
    /** @type {string[]} */
    const exemptTags = ['slot', 'template']
    ...
    return utils.defineTemplateBodyVisitor(context, {
       ...
       */
      VElement(node) {
        if (exemptTags.includes(node.rawName)) {
          return
        }
