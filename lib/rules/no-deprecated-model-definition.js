...
module.exports = {
  ...
  create(context) {
    ...
    return executeOnVue(context, (obj) => {
      const modelProperty = findProperty(obj, 'model')
      ...
      if (
        !propName ||
        !eventName ||
        !allowedPropNames.has(getStringLiteralValue(propName, true) ?? '') ||
        !allowedEventNames.has(getStringLiteralValue(eventName, true) ?? '')
      ) {
        context.report({
          node: modelProperty,
