...
module.exports = {
  ...
  create(context) {
    ...
    const [userMaxCodeLength, userTabWidth, options] = getRuleOptions(
      context,
      __filename
    )

    if (typeof userMaxCodeLength === 'number') {
      options.code = userMaxCodeLength
    } else {
      Object.assign(options, userMaxCodeLength)
    }

    if (typeof userTabWidth === 'number') {
      options.tabWidth = userTabWidth
    } else {
      Object.assign(options, userTabWidth)
    }

    /** @type {number} */
    const scriptMaxLength = options.code
    /** @type {number} */
