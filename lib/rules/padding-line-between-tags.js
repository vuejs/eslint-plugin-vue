...
function removeExcessLines(context, endTag, sibling) {
  context.report({
    ...
    fix(fixer) {
      ...
      let textBetween = splitLines(paddingText);
      let newTextBetween = `\n${textBetween.pop()}`
...
function checkNewline(context) {
  ...
  // Copied from padding-line-between-tags
        ...
        if (configure.blankLine === 'always') {
          if (lineDifference === 1) {
            ...
            context.report({
              ...
              // @ts-ignore
              fix(fixer) {
                const lastSpaces = /** @type {RegExpExecArray} */(/^\s*/.exec(context.getSourceCode().lines[endTag.loc.start.line-1]))[0]
          ...
          if (lineDifference > 1) {
            let hasOnlyTextBetween = false;
            for (let i = endTag.loc.start.line; i < closestSibling.loc.start.line - 1; i++) {
