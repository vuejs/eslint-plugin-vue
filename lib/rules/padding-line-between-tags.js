    ...
    context.report({
      ...
      fix(fixer) {
        const lastSpaces = /** @type {RegExpExecArray} */ (
          /^\s*/.exec(context.sourceCode.lines[endTag.loc.start.line - 1])
        )[0]
...
function removeExcessLines(context, endTag, sibling, lineDifference) {
  if (lineDifference > 1) {
    ...
    for (
      ...
      hasOnlyTextBetween = !/^\s*$/.test(context.sourceCode.lines[i])
    }
...
function matchesConfiguration(configure, prevElement, nextElement) {
  const prevIsSingleLine = isSingleLine(prevElement)
  const nextIsSingleLine = isSingleLine(nextElement)

  // Check prev conditions
  let prevMatches = false
  if (configure.prev !== undefined) {
    prevMatches = configure.prev === '*' || prevElement.name === configure.prev
  } else if (configure.prevSingle !== undefined) {
    prevMatches =
      prevIsSingleLine &&
      (configure.prevSingle === '*' ||
        prevElement.name === configure.prevSingle)
  } else if (configure.prevMulti !== undefined) {
    prevMatches =
      !prevIsSingleLine &&
      (configure.prevMulti === '*' || prevElement.name === configure.prevMulti)
  }

  // Check next conditions
  let nextMatches = false
  if (configure.next !== undefined) {
    nextMatches = configure.next === '*' || nextElement.name === configure.next
  } else if (configure.nextSingle !== undefined) {
    nextMatches =
      nextIsSingleLine &&
      (configure.nextSingle === '*' ||
        nextElement.name === configure.nextSingle)
  } else if (configure.nextMulti !== undefined) {
    nextMatches =
      !nextIsSingleLine &&
      (configure.nextMulti === '*' || nextElement.name === configure.nextMulti)
  }
...
function checkNewline(context) {
  /** @type {Array<{blankLine: "always" | "never" | "consistent", prev?: string, next?: string, prevSingle?: string, prevMulti?: string, nextSingle?: string, nextMulti?: string}>} */
  ...
  return (block) => {
    ...
    const configure = reverseConfigureList.find((configure) =>
      matchesConfiguration(configure, block, closestSibling)
    )
    ...
    schema: [
      {
        ...
        items: {
          ...
          properties: {
            ...
            prev: { type: 'string' },
            next: { type: 'string' },
            prevSingle: { type: 'string' },
            prevMulti: { type: 'string' },
            nextSingle: { type: 'string' },
            nextMulti: { type: 'string' }
          },
          ...
          oneOf: [
            ...
            {
              required: ['prevSingle', 'next']
            },
            {
              required: ['prevMulti', 'next']
            },
            {
              required: ['prev', 'nextSingle']
            },
            {
              required: ['prev', 'nextMulti']
            },
            {
              required: ['prevSingle', 'nextSingle']
            },
            {
              required: ['prevSingle', 'nextMulti']
            },
            {
              required: ['prevMulti', 'nextSingle']
            },
            {
              required: ['prevMulti', 'nextMulti']
            }
          ]
        }
      }
