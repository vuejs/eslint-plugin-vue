...
module.exports = {
   ...
   */
  create(context) {
    const [order, options] = getRuleOptions(context, __filename)

    /** @type {Set<string>} */
    const ignoreGrandchildrenOf = new Set(options.ignoreGrandchildrenOf)
    /** @type {Set<string>} */
    const ignoreChildrenOf = new Set(options.ignoreChildrenOf)
    const insensitive = !options.caseSensitive
    const { minKeys, natural } = options
    const isValidOrder =
