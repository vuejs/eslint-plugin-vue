      ...
      context,
      {
        ...
        "VElement[name='template'][parent.type='VDocumentFragment']:exit"() {
          ...
          const registeredComponentNames = new Set(
            registeredComponents.map(({ name }) => casing.kebabCase(name))
          )

          // All registered components using kebab-case syntax
          const componentsRegisteredAsKebabCase = new Set(
            registeredComponents.flatMap(({ name }) =>
              name === casing.kebabCase(name) ? [name] : []
            )
          )

          const reportNodes = usedComponentNodes.filter(({ name }) => {
            ...
            if (
              ...
              componentsRegisteredAsKebabCase.has(kebabCaseName)
            ) {
              return true
            }

            // Otherwise
            return !registeredComponentNames.has(kebabCaseName)
          })
          for (const { node, name } of reportNodes) {
            context.report({
              node,
              message:
