...
function getSlotDirectivesOnElement(node) {
  return getDirectives(node, 'slot')
}
...
function getSlotDirectivesOnChildren(node) {
  /** @type {VDirective[][]} */
  const groups = []
  for (const group of iterateChildElementsChains(node)) {
    const slotDirs = group
      .map((childElement) =>
        childElement.name === 'template'
          ? getDirective(childElement, 'slot')
          : null
      )
      .filter(isDef)
...
function equalVSlotVForVariables(a, b, tokenStore) {
  ...
  function equal(a, b) {
    ...
    return equalTokens(a, b, tokenStore)
  }
}
...
export default {
  ...
  create(context) {
    ...
    const shouldAllowModifiers = options.allowModifiers === true
    ...
    return defineTemplateBodyVisitor(context, {
      ...
      "VAttribute[directive=true][key.name.name='slot']"(node) {
        ...
        // Verify location.
        if (!isCustomComponent(ownerElement)) {
          context.report({
            node,
        ...
        if (ownerElement === parentElement) {
          const vFor = getDirective(element, 'for')
        ...
        if (hasInvalidModifiers(node, shouldAllowModifiers) && lastModifier) {
