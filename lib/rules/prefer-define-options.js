...
module.exports = {
  ...
  create(context) {
    ...
    /** @type {Set<ImportDeclaration>} */
    const importsInScriptSetup = new Set()

    return utils.compositingVisitors(
      utils.defineScriptSetupVisitor(context, {
        ImportDeclaration(node) {
          importsInScriptSetup.add(node)
        },
    ...
    function buildFix(node, scriptSetup) {
      return (fixer) => {
        ...
        let insertAfterTag = scriptSetup.startTag

        if (importsInScriptSetup.size > 0) {
          const lastImportDeclaration = [...importsInScriptSetup].pop()
          if (lastImportDeclaration) {
            insertAfterTag = lastImportDeclaration
          }
        }
