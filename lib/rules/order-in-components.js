...
const { traverseNodes } = vueEslintParser.AST
...
// Node types that are known to have no side effects.
const NO_SIDE_EFFECT_NODE_TYPES = new Set([
  'FunctionExpression',
  'Identifier',
  'Literal',
  // es2015
...
function isNotSideEffectsNode(node, visitorKeys) {
  ...
  traverseNodes(node, {
    ...
    /** @param {ASTNode} node */
    enterNode(node) {
      if (!isValidOrder || skipNode) {
        return
      }

      if (NO_SIDE_EFFECT_NODE_TYPES.has(node.type)) {
        skipNode = node
      } else if (
        node.type !== 'Property' &&
...
export default {
  ...
  create(context) {
    ...
    function checkOrder(propertiesNodes) {
      const properties = propertiesNodes.filter(isProperty).map((property) => ({
        node: property,
        name:
