  ...
  if (node.parent && node.parent.type === 'VElement') {
    ...
    if (
      ...
      (utils.hasDirective(node, 'bind', 'key') ||
        ...
        !hasFormedConditionalFamily(conditionalFamily))
    ) {
      return
    }
...
/**
 * Checks whether a conditional family has formed.
 * For a conditional family to form it must have an 'if' contdition and either an 'else' condition or at least one 'else-if' condition.
 *
 * @param {ConditionalFamily} family - The conditional family object with 'if', 'else-if', and 'else' properties.
 * @returns {boolean} True if a conditional family has formed, false otherwise.
 */
const hasFormedConditionalFamily = (family) =>
  Boolean(family.else || family.elseIf.length > 0)
