...
module.exports = {
  meta: {
    docs: {
      ...
      url: 'https://github.com/vuejs/eslint-plugin-vue/blob/v4.2.0/docs/rules/mustache-interpolation-spacing.md'
    },
    fixable: 'whitespace',
    schema: [
      {
  ...
  create (context) {
    ...
    return utils.defineTemplateBodyVisitor(context, {
      'VExpressionContainer[expression!=null]' (node) {
        ...
        } else if (options === 'consistent') {
          if (
            (openBrace.range[1] === firstToken.range[0] && closeBrace.range[0] !== lastToken.range[1]) ||
            (openBrace.range[1] !== firstToken.range[0] && closeBrace.range[0] === lastToken.range[1])
          ) {
            context.report({
              node: closeBrace,
              message: openBrace.range[1] === firstToken.range[0]
                ? "Expected no space before '}}', but found."
                : "Expected 1 space before '}}', but not found.",
              fix: (fixer) => {
                if (openBrace.range[1] === firstToken.range[0]) {
                  return fixer.removeRange([lastToken.range[1], closeBrace.range[0]])
                } else if (openBrace.range[1] !== firstToken.range[0]) {
