...
function checkChildKey(context, vFor, child) {
  const childFor = getDirective(child, 'for')
...
function checkKey(context, vFor, element) {
  const vBindKey = getDirective(element, 'bind', 'key')
  ...
  if (isCustomComponent(element) && vBindKey == null) {
    context.report({
...
export default {
  ...
  create(context) {
    ...
    const shouldAllowEmptyAlias = options.allowEmptyAlias === true
    ...
    return defineTemplateBodyVisitor(context, {
      ...
      "VAttribute[directive=true][key.name.name='for']"(node) {
        ...
        if (value === null && !shouldAllowEmptyAlias) {
          context.report({
            node: expr,
