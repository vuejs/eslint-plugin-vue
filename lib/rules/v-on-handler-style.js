...
module.exports = {
  ...
  create(context) {
    const [allowsOption, { ignoreIncludesComment }] =
      /** @type {[HandlerKind | HandlerKind[], { ignoreIncludesComment: boolean }]} */ (
        getRuleOptions(context, __filename)
      )
    const allows = Array.isArray(allowsOption) ? allowsOption : [allowsOption]
