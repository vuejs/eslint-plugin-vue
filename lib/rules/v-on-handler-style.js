...
function parseOptions(context) {
  ...
  const shouldIgnoreIncludesComment = !!option.ignoreIncludesComment

  return { allows, shouldIgnoreIncludesComment }
}
...
export default {
  ...
  create(context) {
    ...
    function verifyCanUseMethodHandlerForInlineHandler(node) {
      ...
      if (shouldIgnoreIncludesComment && hasComment) {
        return false
      }
      ...
      context.report({
        ...
        fix: (fixer) => {
          ...
          return fixer.replaceTextRange(
            rangeWithoutQuotes,
            sourceCode.getText(idCallExpr.callee)
          )
        }
      })
      return true
    ...
    function reportCanUseInlineFunctionForInlineHandler(node) {
      context.report({
        ...
        *fix(fixer) {
          // eslint-disable-next-line unicorn/consistent-boolean-name -- `has` prefix is present; the `$` refers to the `$event` argument
        ...
        ? // Collect method definition with params information to check for side effects.
          ...
          defineVueVisitor(context, {
            onVueObjectEnter(node) {
              const methods = iterateProperties(node, new Set(['methods']))
