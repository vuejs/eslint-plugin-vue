...
module.exports = {
  ...
  create(context) {
    ...
    const methodArgumentCountMap = new Map()
    /** @type {Identifier[]} */
    const $eventIdentifiers = []

    /**
     * @typedef {object} ReportResult
     * @property {import('eslint').ReportDescriptorFix | null} fix
    ...
    function verifyInlineHandlerCanBeReplacedWithMethodHandler(node) {
      ...
      const argumentsCount = methodArgumentCountMap.get(expression.callee.name)
      if (argumentsCount != null && argumentsCount > 0) {
      ...
      return {
        ...
        fix: hasComment
          ? null /* The statement contains comment and cannot be fixed. */
          : (fixer) =>
    ...
    function verifyInlineFunctionHandlerCanBeReplacedWithMethodHandler(node) {
      ...
      if (node.body.type === 'BlockStatement') {
        ...
        if (!isIdentifierCallExpression(node.body)) {
          return null
        }
      ...
      if (
        argumentsCount != null &&
        argumentsCount !== expression.arguments.length
      ) {
        // The behavior of target method can change given the arguments.
        return null
      }
      ...
      return {
        ...
        fix: hasComment
          ? null /* The function contains comment and cannot be fixed. */
          : (fixer) =>
              fixer.replaceTextRange(
                innerRange,
                context.getSourceCode().getText(expression.callee)
    ...
    return utils.defineTemplateBodyVisitor(
      ...
      allows.includes('method')
        ? // Collect method definition with arguments information to check for side effects.
