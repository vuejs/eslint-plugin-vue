...
import reservedNames from '../utils/js-reserved.json' with { type: 'json' }

const RESERVED_NAMES = new Set(reservedNames)
      ...
      VElement(node) {
        ...
        if (node.variables) {
          for (const variable of node.variables) {
            ...
            if (scopeStack.nodes.every((node) => node.name !== name)) {
      ...
      ...(options === 'never'
        ? {
            ...
            'VExpressionContainer MemberExpression > ThisExpression'(node) {
              ...
              const propertyName = getStaticPropertyName(node.parent)
              if (
                !propertyName ||
