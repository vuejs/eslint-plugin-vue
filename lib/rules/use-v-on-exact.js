...
import {
  ...
  isCustomComponent as utilsIsCustomComponent
} from '../utils/index.js'
...
function getEventDirectives(startTag, sourceCode) {
  return getDirectives(startTag, 'on').map((attribute) => ({
...
function isGlobalOnlyKeyboardStopEvent(event) {
  return (
    KEYBOARD_EVENTS.has(event.name) &&
    event.modifiers.includes('stop') &&
    !event.hasHandler &&
    event.modifiers.every((modifier) => GLOBAL_MODIFIERS.has(modifier))
  )
}
...
function hasConflictedModifiers(baseEvent, event) {
  ...
  if (
    baseEvent.modifiers.includes('exact') &&
    isGlobalOnlyKeyboardStopEvent(event)
  )
    return false
  ...
  create(context) {
    ...
    return defineTemplateBodyVisitor(context, {
      ...
      VStartTag(node) {
        ...
        const isCustomComponent = utilsIsCustomComponent(node.parent)
