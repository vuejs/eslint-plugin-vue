  ...
  create(context) {
    ...
    const templateVisitor = {
      ...
      VExpressionContainer(node) {
        ...
        if(node.expression != null && node.expression.callee != null){
          // Stuck here don't understand how to iterate over this
        }
      },
      /**
      ...
      /**
       * @param {VAttribute} node // tried using this but didn't find any good solution
       */
      // 'VAttribute[directive=true]'(node) {
      //   if (node.key.name === 'bind' && node.value != null && node.value.expression != null) {
