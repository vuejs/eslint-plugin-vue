...
const { isJSDocComment } = require('../utils/comments.ts')
const {
  getStyleVariablesContext
} = require('../utils/style-variables/index.ts')
const {
  definePropertyReferenceExtractor,
  mergePropertyReferences
} = require('../utils/property-references.ts')
    ...
    const scriptVisitor = compositingVisitors(
      ...
      defineVueVisitor(context, {
        ...
        onVueObjectEnter(node, vueNode) {
          ...
          for (const watcherOrExpose of iterateProperties(
            node,
