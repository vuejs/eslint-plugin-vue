...
/**
 * Normalize options.
 * @param {UserPreferOption} preferOptions The options user configured.
 * @returns {NormalizeOptions} The normalized options.
 */
function parseOptions(preferOptions) {
...
module.exports = {
  ...
  create(context) {
    const [preferOptions] = getRuleOptions(context, __filename)
    const options = parseOptions(preferOptions)
