      ...
      compositingVisitors(
        ...
        defineVueVisitor(context, {
          onVueObjectEnter(node) {
            const setupProperties = iterateProperties(node, new Set(['setup']))
        ...
        }),
        {
          Identifier(id) {
            ...
            const refsNode =
              id.parent.type === 'MemberExpression' && id.parent.property === id
                ? id.parent // `this.$refs.foo`
                : id
          ...
          CallExpression(callExpression) {
            ...
            const name = getStringLiteralValue(firstArgument)
            if (name !== null) {
              usedRefs.add(name)
            }
          }
