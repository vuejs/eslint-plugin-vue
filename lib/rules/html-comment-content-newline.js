...
module.exports = {
  ...
  create(context) {
    const [option, config] = getRuleOptions(context, __filename)
    const { singleline, multiline } = parseOption(option)
    return htmlComments.defineVisitor(context, config, (comment) => {
      ...
      const endLine = closeDecoration
        ...
        : value.loc.end.line
      const newlineType = startLine === endLine ? singleline : multiline
