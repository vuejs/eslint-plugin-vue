  ...
  create(context) {
    ...
    const handleObjectProp = (prop, defaultProps) => {
      if (
        ...
        (findProperty(prop.value, 'default') ||
          defaultProps?.has(prop.propName))
      ) {
        const requiredProperty = findProperty(prop.value, 'required')
    ...
    return compositingVisitors(
      defineVueVisitor(context, {
        onVueObjectEnter(node) {
          for (const prop of getComponentPropsFromOptions(node)) {
            handleObjectProp(prop)
          }
        }
      }),
      defineScriptSetupVisitor(context, {
        onDefinePropsEnter(node, props) {
          const defaultProps = new Set([
            ...Object.keys(getWithDefaultsPropExpressions(node)),
            ...Object.keys(getDefaultPropExpressionsForPropsDestructure(node))
          ])
          for (const prop of props) {
            handleObjectProp(prop, defaultProps)
          }
        }
      })
    )
