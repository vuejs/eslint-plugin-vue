...
import {
  ...
  hasWithDefaults as utilsHasWithDefaults,
  getWithDefaultsPropExpressions,
  isUsingPropsDestructure as utilsIsUsingPropsDestructure,
...
function propIsRequired(propValue) {
  const propRequiredNode = propValue.properties.find(
    (p) =>
      p.type === 'Property' &&
      getStaticPropertyName(p) === 'required' &&
...
function propHasDefault(propValue) {
  const propDefaultNode = propValue.properties.find(
    (p) => p.type === 'Property' && getStaticPropertyName(p) === 'default'
  )
...
function isWithoutDefaultValue(prop) {
  if (prop.value.type !== 'ObjectExpression') {
    ...
    // `CallExpression`/`MemberExpression` are OK; anything else is NG.
    return (
        ...
        if (
          ...
          options.properties.some(
            (property) =>
              ...
              getStaticPropertyName(property) === 'type' &&
              isValueNodeOfBooleanType(property.value)
          )
        ) {
          return true
        }
    ...
    return compositingVisitors(
      defineScriptSetupVisitor(context, {
        onDefinePropsEnter(node, props) {
          const hasWithDefaults = utilsHasWithDefaults(node)
          const defaultsByWithDefaults = getWithDefaultsPropExpressions(node)
          const isUsingPropsDestructure = utilsIsUsingPropsDestructure(node)
