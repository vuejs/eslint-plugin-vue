...
module.exports = {
  ...
  create(context) {
    ...
    /** @type {{(comments:Comment[]):string|undefined}} */
    const verifyBlock = (comments) => {
      if (comments.length > 0 && comments[0].type === 'Block') return undefined
      return 'The "{{name}}" property should have a block comment.'
    }
    /** @type {{(comments:Comment[]):string|undefined}} */
    ...
    const verifyJSDoc = (comments) => {
      if (
        ...
        return ''
      return 'The "{{name}}" property should have a JSDoc comment.'
    }
