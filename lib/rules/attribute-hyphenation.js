...
module.exports = {
  ...
  create(context) {
    ...
    const ignoreComponents =
      (optionsPayload && optionsPayload.ignoreComponents) || []

    const caseConverter = casing.getExactConverter(
      useHyphenated ? 'kebab-case' : 'camelCase'
    )

    const toKebab = casing.getExactConverter('kebab-case')
     ...
     */
    function isIgnoredAttribute(value, tagname) {
      const isIgnored =
        ignoredAttributes.some((attr) => {
          return value.indexOf(attr) !== -1
        }) ||
        ignoreComponents.some((tag) => {
    ...
    return utils.defineTemplateBodyVisitor(context, {
      VAttribute(node) {
        ...
        if (
          !name ||
          isIgnoredAttribute(name, toKebab(node.parent.parent.rawName))
        ) {
          return
        }
