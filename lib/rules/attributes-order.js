...
function getPosition (attribute, attributePosition) {
  const attributeType = attribute.directive && attribute.key.name.name === 'bind'
    ? getAttributeType(attribute.key.argument && attribute.key.argument.name, false)
    : getAttributeType(attribute.directive ? attribute.key.name.name : attribute.key.name, attribute.directive)
