...
const { getVueComponentDefinitionType } = require('../utils/index.ts')
...
function isExposeProperty(node) {
  return (
    node.type === 'Property' &&
    getStaticPropertyName(node) === 'expose' &&
    !node.computed
  )
}
    ...
    return defineVueVisitor(context, {
      onSetupFunctionEnter(node, { node: vueNode }) {
        ...
        if (contextParam.type === 'ObjectPattern') {
          const exposeProperty = findAssignmentProperty(contextParam, 'expose')
          if (!exposeProperty) {
            return
          }
...
function buildSuggest(object, context) {
  ...
  const beforeOptionNode = propertyNodes.find((p) =>
    FIX_EXPOSE_BEFORE_OPTIONS.has(getStaticPropertyName(p) || '')
  )
  const allProps = [
    ...new Set(
      iterateProperties(
