...
function parseDeclaration(code) {
  ...
  if (componentDeclaration) {
    componets.push(...Object.keys(extractImports(componentDeclaration)))
  }
 ...
 * @returns
 */
function parseGlobalComponents(globals, cwd) {
  const globalComponets = []
  for (const item of globals) {
    if (!item.endsWith('.d.ts')) {
      globalComponets.push(item)
    }
  }
  const paths = globals
    .filter((i) => i.endsWith('.d.ts'))
    .map((p) => path.resolve(cwd, p))
  const autoImportNames = paths
    .filter((p) => fs.existsSync(p))
    .map((p) => fs.readFileSync(p, 'utf-8'))
    .map(parseDeclaration)
    .flat()
  globalComponets.push(...autoImportNames)
  return globalComponets
}
...
module.exports = {
  ...
  create(context) {
    ...
    /** @type {string[]} */
    const globals = parseGlobalComponents(
      options.globals || [],
      context.cwd || process.cwd()
    ).map(casing.pascalCase)
