...
function checkKey (context, element) {
  assert(element && element.type === 'VElement')
  const startTag = element.startTag

  if (element.name !== 'template' && !utils.isCustomComponent(startTag) && !utils.hasDirective(startTag, 'bind', 'key')) {
    context.report({
      node: startTag,
      loc: startTag.loc,
