...
module.exports = {
  ...
  /**
   * Returns the list of all registered components in a class declaration
   * @param {ObjectExpression} componentObject 
   * @returns { { node: Property, name: string }[] } Array of ASTNodes
   */
  getRegisteredComponentsFromClassDeclaration(componentObject){
    const decorators = componentObject.decorators
    ...
    return hasArguments ? this.getRegisteredComponents(expression.arguments[0]) : []
  },
...
function getVueComponentDefinitionType(node) {
  ...
  if (parent.type === 'CallExpression') {
    ...
    if (callee.type === 'Identifier') {
      if (callee.name === 'component' || callee.name === 'Component') {
        // for Vue.js 2.x
        // component('xxx', {})
