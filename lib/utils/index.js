...
module.exports = {
  ...
  isCustomComponent (node) {
    ...
    return (
            !(this.isHtmlElementName(name) || this.isSvgElementName(name) || this.isVueElementName(name)) ||
  ...
  isVoidElementName (name) {
    ...
    /**
     * Check whether the given name is a vue element name or not.
     * @param {string} name The name to check.
     * @returns {boolean} `true` if the name is a vue element name.
     */
  isVueElementName (name) {
    assert(typeof name === 'string')

    return VUE_ELEMENT_NAMES.has(name.toLowerCase())
  }
}
