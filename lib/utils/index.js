...
module.exports = {
  ...
  *iterateChildElementsChains(node) {
   ...
   * @returns {'component' | 'mixin' | 'mixins' | 'extend' | 'createApp' | 'defineComponent' | null}
   */
...
function getVueComponentDefinitionType(node) {
  ...
  if (parent.type === 'CallExpression') {
    ...
    if (callee.type === 'MemberExpression') {
      const calleeObject = skipTSAsExpression(callee.object)
      if (calleeObject.type === 'CallExpression' && calleeObject.callee.type === 'Identifier') {
        const propName = calleeObject.callee.name

        // for vue-typed-mixins Vue.js 2.x
        // mixins({})
        const maybeFullVueTypedMixinComponentForVue2 =
          propName && isObjectArgument(parent)

        return maybeFullVueTypedMixinComponentForVue2 &&
            propName === 'mixins'
          ? propName
          : null
      }
