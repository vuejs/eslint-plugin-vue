...
import vue2BuiltinComponents from './vue2-builtin-components.js'
import vue3BuiltinComponents from './vue3-builtin-components.js'
import vueBuiltinElements from './vue-builtin-elements.js'
...
export const VUE2_BUILTIN_COMPONENT_NAMES = new Set(vue2BuiltinComponents)
export const VUE3_BUILTIN_COMPONENT_NAMES = new Set(vue3BuiltinComponents)
 ...
 */
export function getCoreRule(name) {
  try {
    // eslint-disable-next-line unicorn/no-top-level-assignment-in-function -- lazily cache the core rule map
    coreRuleMap ||= new Linter().getRules()
 ...
 */
export function wrapCoreRule(coreRuleName, options) {
 ...
 * @returns {RuleModule} The wrapped rule implementation.
 */
export function wrapStylisticOrCoreRule(ruleNames, options) {
 ...
 */
export function isEmptyValueDirective(node, context) {
  if (node.value == null) {
    return false
  }
 ...
 */
export function isEmptyExpressionValueDirective(node, context) {
  if (node.value == null) {
    return false
  }
  if (node.value.expression != null) {
 ...
 */
export function getRegisteredComponents(componentObject) {
 ...
 */
export function prevElementHasIf(node) {
 ...
 */
export function* iterateChildElementsChains(node) {
  let isVIf = false
  /** @type {VElement[]} */
  let elementChain = []
 ...
 */
export function isCustomComponent(node, shouldIgnoreElementNamespaces = false) {
  if (
  ...
  if (shouldIgnoreElementNamespaces) {
 ...
 */
export function isSvgElementNode(node) {
 ...
 */
export function isMathElementNode(node) {
 ...
 */
export function isHtmlWellKnownElementName(name) {
 ...
 */
export function isSvgWellKnownElementName(name) {
 ...
 */
export function isMathWellKnownElementName(name) {
 ...
 */
export function isHtmlVoidElementName(name) {
 ...
 */
export function isBuiltInComponentName(name) {
  return (
 ...
 */
export function isVueBuiltInElementName(name) {
 ...
 */
export function isBuiltInDirectiveName(name) {
 ...
 */
export function getComputedProperties(componentObject) {
 ...
 */
export function getGetterBodyFromComputedFunction(callExpression) {
 ...
 */
export function executeOnVue(context, cb) {
 ...
 */
export function defineVueVisitor(context, visitor) {
 ...
 */
export function defineScriptSetupVisitor(context, visitor) {
  ...
  function callVisitor(key, node, ...args) {
    if (visitor[key] && (node.type === 'Program' || isInScriptSetup(node))) {
      // @ts-expect-error
 ...
 */
export function getWithDefaultsPropExpressions(node) {
 ...
 * @returns {boolean}
 */
export function isUsingPropsDestructure(node) {
 ...
 * @param { (node: ObjectExpression, type: VueObjectType) => void } cb Callback function
 */
export function executeOnVueInstance(context, cb) {
  return {
    /** @param {ObjectExpression} node */
 ...
 * @param { (node: ObjectExpression, type: VueObjectType) => void } cb Callback function
 */
export function executeOnVueComponent(context, cb) {
  return {
    /** @param {ObjectExpression} node */
    'ObjectExpression:exit'(node) {
      const type = getVueObjectType(context, node)
 ...
 */
export function executeOnCallVueComponent(_context, cb) {
  return {
 ...
 */
export function* iterateProperties(node, groups) {
  for (const item of node.properties) {
    ...
    yield* iteratePropertyValue(item.value, name)
  }
}
 ...
 * @returns {IterableIterator<ComponentPropertyData>}
 */
export function* iteratePropertyValue(value, groupName) {
  switch (value.type) {
    case 'ArrayExpression': {
      yield* iterateArrayExpression(value, groupName)
      break
    }
    case 'ObjectExpression': {
      yield* iterateObjectExpression(value, groupName)
      break
    }
    case 'FunctionExpression': {
      yield* iterateFunctionExpression(value, groupName)
      break
    }
    case 'ArrowFunctionExpression': {
      yield* iterateArrowFunctionExpression(value, groupName)
      break
    }
  }
}
 ...
 */
export function* iterateArrayExpression(node, groupName) {
 ...
 */
export function* iterateObjectExpression(node, groupName) {
  ...
  for (const item of node.properties) {
    ...
    const key = item.key
    if (
      // eslint-disable-next-line unicorn/prefer-includes-over-repeated-comparisons -- narrows `key`'s type for the `yield` below
      key.type === 'Identifier' ||
      key.type === 'Literal' ||
 ...
 * @returns {IterableIterator<ComponentObjectPropertyData>}
 */
export function* iterateFunctionExpression(node, groupName) {
 ...
 * @returns {IterableIterator<ComponentObjectPropertyData>}
 */
export function* iterateArrowFunctionExpression(node, groupName) {
...
export function executeOnFunctionsWithoutReturn(
  ...
  // "Spurious" = ESLint sees the segment as reachable, but TypeScript proves
  // it can't actually execute (the phantom no-match edge of a type-exhaustive
  // switch with no default).
  /** @type {WeakSet<CodePathSegment>} */
  const spuriousSegments = new WeakSet()

  // Segments started inside a switch case body. Lets us distinguish post-switch
  // reachability via a real path (break, fall-through) from the phantom no-match
  // path. Depth is a counter to support nested switches.
  /** @type {WeakSet<CodePathSegment>} */
  const caseBodySegments = new WeakSet()
  let caseBodyDepth = 0
  ...
  function isValidReturn() {
    ...
    if (
      funcInfo.currentSegments.some(
        (segment) => !isEffectivelyUnreachable(segment)
      )
    ) {
      return false
    }
  ...
  return {
    ...
    onCodePathStart(codePath, node) {
      if (
        // eslint-disable-next-line unicorn/prefer-includes-over-repeated-comparisons -- narrows `node`'s type for `funcInfo` below
    ...
    onCodePathSegmentStart(segment) {
      ...
      // Spurious propagates: if every reachable predecessor is spurious, so is this segment.
      if (
        segment.prevSegments.length > 0 &&
        segment.prevSegments.every(isEffectivelyUnreachable)
      ) {
        spuriousSegments.add(segment)
      }
      if (caseBodyDepth > 0) {
        caseBodySegments.add(segment)
      }
    },
    ...
    'SwitchStatement:exit'(node) {
      if (!funcInfo) return
      // Switches with `default` are handled correctly by ESLint already.
      if (node.cases.some((switchCase) => switchCase.test === null)) return
      if (!isExhaustiveSwitch(context, node)) return

      // If any reachable predecessor came from inside a case body, the switch
      // genuinely falls through (break or fall-through). Otherwise the only
      // incoming edge is the phantom no-match path — safe to mark spurious.
      const fromCaseBody = funcInfo.currentSegments.some((post) =>
        post.prevSegments.some(
          (prev) =>
            !isEffectivelyUnreachable(prev) && caseBodySegments.has(prev)
        )
      )
      if (fromCaseBody) return

      for (const segment of funcInfo.currentSegments) {
        spuriousSegments.add(segment)
      }
    },
 ...
 */
export function hasInvalidEOF(node) {
 ...
 */
export function getMemberChaining(node) {
 ...
 */
export function inRange(rangeOrNode, target) {
 ...
 */
export function isThis(node, context) {
 ...
 * @returns {boolean}
 */
export function isObjectAssignCall(callExpr, targetNode) {
 ...
 */
export function findMutating(props) {
  /** @type {MemberExpression[]} */
  const pathNodes = []
  ...
  while (true) {
    const step = findMutatingStep(node, target, pathNodes)
    if (step.done) {
      return step.result
    }
    node = step.node
 ...
 */
export function findMutatingStep(node, target, pathNodes) {
  switch (target.type) {
    case 'AssignmentExpression': {
      if (target.left === node) {
        ...
        return {
          ...
          result: { kind: 'assignment', node: target, pathNodes }
        }
      }
    ...
    case 'UpdateExpression': {
      ...
      return {
        ...
        result: { kind: 'update', node: target, pathNodes }
      }
    }
      ...
      if (mem && target.callee === node) {
        ...
        if (
          ...
          return {
            ...
            result: { kind: 'call', node: target, pathNodes }
          }
        }
      }
    ...
    case 'MemberExpression': {
      if (target.object === node) {
        ...
        return { done: false, node: target, target: target.parent }
      }
      return { done: true, result: null }
    }
 ...
 */
export function* iterateReferencesTraceMap(tracker, map) {
 ...
 * @returns {RuleListener} The merged visitor.
 */
export function defineDocumentVisitor(context, documentVisitor, options) {
  const sourceCode = context.sourceCode
 ...
 * @returns {T}
 */
export function compositingVisitors(visitor, ...visitors) {
 ...
 */
export function findVariableByIdentifier(context, node) {
 ...
 */
export function findProperty(node, name, filter) {
 ...
 */
export function findAssignmentProperty(node, name, filter) {
  const predicate = filter
    ? /**
 ...
 */
export function isAssignmentProperty(node) {
 ...
 */
export function isVElement(node) {
 ...
 * @returns {boolean}
 */
export function isInExportDefault(node) {
  /** @type {ASTNode | null} */
 ...
 */
export function skipTSAsExpression(node) {
  if (!node) {
    return node
  }
  // @ts-expect-error
 ...
 * @param {Expression} node
 */
export function isPropertyChain(prop, node) {
  let value = node
 ...
 */
export function skipDefaultParamValue(node) {
  if (!node) {
    return node
  }
  // @ts-expect-error
 ...
 * @returns {boolean}
 */
export function withinTypeNode(node) {
  /** @type {ASTNode | null} */
  let target = node
  while (target) {
 ...
 */
export function getStringLiteralValue(node, stringOnly) {
 ...
 */
export function getNameParamNode(node) {
 ...
 * @param {RuleContext} context The ESLint rule context object.
 */
export function isScriptSetup(context) {
 ...
 */
export function getScriptSetupElement(context) {
  const sourceCode = context.sourceCode
  const df =
...
function isGlobalOrImportedFromVue(callee, context) {
  ...
  if (variable !== null && variable.defs.length === 1) {
    ...
    const vueLikePackageNames = new Set([
      'vue',
      '@vue/composition-api',
    ...
    if (
      ...
      return vueLikePackageNames.has(def.node.parent.source.value)
    }
    ...
    // name(); let name = ...
    if (
      ['CatchClause', 'ClassName', 'FunctionName', 'TDZ'].includes(def.type)
    ) {
      return false
    }
  }
 ...
 */
export function getVueComponentDefinitionType(context, node) {
  ...
  if (parent.type === 'CallExpression') {
    ...
    if (callee.type === 'MemberExpression') {
      ...
      if (calleeObject.type === 'Identifier') {
        ...
        if (calleeObject.name === 'Vue') {
          ...
          return maybeFullVueComponentForVue2 &&
            // eslint-disable-next-line unicorn/prefer-includes-over-repeated-comparisons -- narrows `propName` for the returned value
            (propName === 'component' ||
              propName === 'mixin' ||
              propName === 'extend')
            ? propName
      ...
      if (
        // for Vue.js 2.x
        // component('xxx', {})
        // eslint-disable-next-line unicorn/prefer-includes-over-repeated-comparisons -- narrows `callee.name` for the returned value
 ...
 */
export function getVueObjectType(context, node) {
 ...
 */
export function isSFCObject(context, node) {
  if (node.type !== 'ObjectExpression') {
    return false
  }
  const filePath = context.filename
  ...
  function isSFC(node) {
    ...
    switch (parent.type) {
      case 'ExportDefaultDeclaration': {
        ...
        const scriptSetup = getScriptSetupElement(context)
        // `false` for `export default` in `<script setup>`
        return (
          !scriptSetup ||
          scriptSetup.range[0] > parent.range[0] ||
          parent.range[1] > scriptSetup.range[1]
 ...
 */
export function* iterateWatchHandlerValues(property) {
  ...
  if (value.type === 'ObjectExpression') {
    ...
    for (const element of value.elements) {
      if (isDef(element) && element.type !== 'SpreadElement') {
 ...
 * @param {import('@eslint-community/eslint-utils').TYPES.TraceMap} map
 */
export function createCompositionApiTraceMap(map) {
  return {
    vue: map,
 ...
 */
export function getAttribute(node, name, value) {
  return (
 ...
 */
export function getDirectives(node, name) {
  const attributes = (node.type === 'VElement' ? node.startTag : node)
    .attributes
 ...
 */
export function getDirective(node, name, argument) {
  return (
    node.startTag.attributes.find(
      /**
       * @param {VAttribute | VDirective} node
 ...
 */
export function hasAttribute(node, name, value) {
 ...
 */
export function hasDirective(node, name, argument) {
 ...
 */
export function isVBindSameNameShorthand(node) {
  return (
    node.directive &&
 ...
 */
export function hasWithDefaults(node) {
  return (
    node.parent &&
 ...
 */
export function getWithDefaultsProps(node) {
 ...
 */
export function getPropsDestructure(node) {
 ...
 */
export function getDefaultPropExpressionsForPropsDestructure(node) {
 ...
 */
export function getLeftOfDefineProps(node) {
  let target = node
 ...
 */
export function getComponentPropsFromOptions(componentObject) {
 ...
 */
export function getComponentEmitsFromOptions(componentObject) {
