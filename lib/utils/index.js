...
module.exports = {
   ...
   * Get all computed properties by looking at all component's properties
   * @param {ObjectExpression} componentObject Object with component definition
   * @return {Array} Array of node: could be all kinds of node
   */
  getDataInnerNode (componentObject) {
    const dataPropertiesNode = componentObject.properties
      .find(p =>
        p.type === 'Property' &&
        p.key.type === 'Identifier' &&
        p.key.name === 'data' &&
        (p.value.type === 'ObjectExpression' || p.value.type === 'FunctionExpression')
      )

    if (!dataPropertiesNode) { return [] }
    let nodeList
    if (dataPropertiesNode.value.type === 'FunctionExpression') {
      const nodeListGenerator = this.iterateFunctionExpression(dataPropertiesNode.value)
      nodeList = [...nodeListGenerator]
    } else {
      nodeList = this.iterateObjectExpression(dataPropertiesNode.value)
    }
    return nodeList
  },
