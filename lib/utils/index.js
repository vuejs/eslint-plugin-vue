...
module.exports = {
  ...
  * iterateFunctionExpression (node, groupName) {
   ...
   * @param {any} b string b to compare
   * @param {number} [threshold=1]
   * @returns if these two string editdistance lessEqual than threshold,
   *          usually if the answer is true, like `method` `methods`, we
   *          could say this is a potential typo.
   */
  isEditDistanceLessEqualThanThreshold (a, b, threshold = 1) {
    const alen = a.length
    const blen = b.length
