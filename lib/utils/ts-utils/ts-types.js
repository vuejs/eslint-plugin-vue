...
/**
 * Check if a statement guarantees return/throw through all execution paths.
 * Handles nested blocks, if/else, switch (via isExhaustiveSwitch), and try/catch.
 * @param {RuleContext} context
 * @param {ESNode} stmt
 * @returns {boolean}
 */
function guaranteesReturn(context, stmt) {
  ...
  if (stmt.type === 'BlockStatement') {
    ...
    return guaranteesReturn(context, /** @type {ESNode} */ (stmts.at(-1)))
  }
  if (stmt.type === 'IfStatement') {
    return (
      stmt.alternate != null &&
      guaranteesReturn(context, stmt.consequent) &&
      guaranteesReturn(context, stmt.alternate)
    )
  }
  if (stmt.type === 'SwitchStatement') {
    return isExhaustiveSwitch(context, stmt)
  }
  if (stmt.type === 'TryStatement') {
    if (stmt.finalizer && guaranteesReturn(context, stmt.finalizer)) return true
    if (!guaranteesReturn(context, stmt.block)) return false
    if (stmt.handler && !guaranteesReturn(context, stmt.handler.body)) {
      return false
    }
    return true
  }
...
function isExhaustiveSwitch(context, switchNode) {
  ...
  for (const switchCase of switchNode.cases) {
    ...
    if (!last || !guaranteesReturn(context, last)) return false
  }
  return true
}

/**
 * Check if a function body guarantees a return or throw on every path,
 * recognizing type-exhaustive switches via TypeScript type information.
 * @param {RuleContext} context The ESLint rule context object.
 * @param {ArrowFunctionExpression | FunctionExpression | FunctionDeclaration} node
 * @returns {boolean}
 */
function hasGuaranteedReturn(context, node) {
  ...
  return lastStmt != null && guaranteesReturn(context, lastStmt)
}
