 ...
 * @returns {boolean}
 */
function hasGuaranteedReturn(context, stmt) {
  ...
  if (stmt.type === 'BlockStatement') {
    ...
    return hasGuaranteedReturn(context, /** @type {ESNode} */ (stmts.at(-1)))
  }
  if (stmt.type === 'IfStatement') {
    return (
      stmt.alternate != null &&
      hasGuaranteedReturn(context, stmt.consequent) &&
      hasGuaranteedReturn(context, stmt.alternate)
    )
  }
  ...
  if (stmt.type === 'TryStatement') {
    if (stmt.finalizer && hasGuaranteedReturn(context, stmt.finalizer))
      return true
    if (!hasGuaranteedReturn(context, stmt.block)) return false
    if (stmt.handler && !hasGuaranteedReturn(context, stmt.handler.body)) {
      return false
    }
    return true
  }
...
function isExhaustiveSwitch(context, switchNode) {
  ...
  const constituents = unionConstituents(discriminantType)
    ...
    .filter((t) => isTypeLiteralLike(t, ts))
  ...
  for (const switchCase of switchNode.cases) {
    ...
    if (!last || !hasGuaranteedReturn(context, last)) return false
  }
  return true
}
