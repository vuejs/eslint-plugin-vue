...
import { inferRuntimeTypeFromTypeNode } from './ts-types.js'
/**
 ...
 */
export function isTSInterfaceBody(node) {
 ...
 */
export function isTSTypeLiteralOrTSFunctionType(node) {
 ...
 */
export function* extractRuntimeProps(context, node) {
 ...
 */
export function* extractRuntimeEmits(node) {
 ...
 */
export function* extractRuntimeSlots(node) {
  const members = node.type === 'TSTypeLiteral' ? node.members : node.body
  for (const member of members) {
    if (
      member.type !== 'TSPropertySignature' &&
 ...
 */
export function flattenTypeNodes(context, node) {
  /**
 ...
 * @returns {string[]}
 */
export function inferRuntimeType(context, node, checked = new Set()) {
  switch (node.type) {
