...
export function getTypeScript() {
  ...
  try {
    // eslint-disable-next-line unicorn/no-top-level-assignment-in-function -- lazily cache the TypeScript module
 ...
 * @returns {string[]}
 */
export function extractTypeFlags(tsType) {
 ...
 * @returns {string[]}
 */
export function extractObjectFlags(tsType) {
  if (!isObject(tsType)) {
    return []
  }
  const ts = /** @type {TypeScript} */ (getTypeScript())
 ...
 */
export function isObject(tsType) {
  const ts = /** @type {TypeScript} */ (getTypeScript())
 ...
 * @param {Type} tsType The type to check.
 */
export function isArrayLikeObject(tsType) {
  const ts = /** @type {TypeScript} */ (getTypeScript())
  return (
    isObject(tsType) &&
    (tsType.objectFlags &
 ...
 * @param {Type} tsType The type to check.
 */
export function isUnknown(tsType) {
  const ts = /** @type {TypeScript} */ (getTypeScript())
 ...
 * @param {Type} tsType The type to check.
 */
export function isNever(tsType) {
  const ts = /** @type {TypeScript} */ (getTypeScript())
 ...
 */
export function isStringLike(tsType) {
  const ts = /** @type {TypeScript} */ (getTypeScript())
 ...
 */
export function isNumberLike(tsType) {
  const ts = /** @type {TypeScript} */ (getTypeScript())
 ...
 */
export function isBooleanLike(tsType) {
  const ts = /** @type {TypeScript} */ (getTypeScript())
 ...
 */
export function isBigIntLike(tsType) {
  const ts = /** @type {TypeScript} */ (getTypeScript())
 ...
 */
export function isReferenceObject(tsType) {
  const ts = /** @type {TypeScript} */ (getTypeScript())
  return (
 ...
 * @param {Type} tsType The type to check.
 */
export function isFunction(tsType) {
  const ts = /** @type {TypeScript} */ (getTypeScript())
  if (
    tsType.symbol &&
    (tsType.symbol.flags &
